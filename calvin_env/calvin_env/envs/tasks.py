...
class Tasks:
    ...
    def rotate_object(
        ...
        if z_degrees > 0:
            success = z_degrees < z < z_threshold and abs(x) < x_y_threshold and abs(y) < x_y_threshold
            if success:
                print(f"[z_degrees > 0] obj_name: {obj_name}, {z_degrees} < {z} < {z_threshold} : {z_degrees < z < z_threshold}, abs({x}) < {x_y_threshold}: {abs(x) < x_y_threshold}, abs({y}) < {x_y_threshold}: {abs(y) < x_y_threshold}")
            return success
        else:
            success = z_degrees > z > -z_threshold and abs(x) < x_y_threshold and abs(y) < x_y_threshold
            if success:
    ...
    def push_object(obj_name, x_direction, y_direction, start_info, end_info):
        ...
        if x_direction > 0:
            success = pos_diff[0] > x_direction
            if success:
                print(f"[{obj_name}] x_direction: {x_direction}")
            return success
        elif x_direction < 0:
            success = pos_diff[0] < x_direction
            if success:
        ...
        if y_direction > 0:
            success = pos_diff[1] > y_direction
            if success:
                print(f"[{obj_name}] y_direction: {y_direction}")
            return success
        elif y_direction < 0:
            success = pos_diff[1] < y_direction
            if success:
    ...
    def lift_object(obj_name, z_direction, surface_body=None, surface_link=None, start_info=None, end_info=None):
        ...
        # print(f"obj_name: {obj_name}, z_direction: {z_direction}, z_diff: {z_diff}, z_diff > z_direction: {z_diff > z_direction}")
        ...
        if success:
            print(f"[{obj_name}] z_direction: {z_direction}")
    ...
    def move_door_abs(joint_name, start_threshold, end_threshold, start_info, end_info):
        ...
        if start_threshold < end_threshold:
            success = start_joint_state < start_threshold < end_threshold < end_joint_state
            if success:
                print(f"[{joint_name}] start_threshold: {start_threshold}, end_threshold: {end_threshold}")
            return success
        elif start_threshold > end_threshold:
            success = start_joint_state > start_threshold > end_threshold > end_joint_state
            if success:
    ...
    def move_door_rel(joint_name, threshold, start_info, end_info):
        ...
        # )
        success = 0 < threshold < end_joint_state - start_joint_state or 0 > threshold > end_joint_state - start_joint_state
        if success:
            print(f"[{joint_name}] threshold: {threshold}")
        return success
